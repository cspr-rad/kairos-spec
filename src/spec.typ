...
The client CLI's user interface (UI) is comprised of commands allowing a user to deposit, transfer, and withdraw funds allocated in their L2 account. Once a user executes any of the commands, the client delegates the bulk of the work to the L2 node (@l2-node). Moreover, the client CLI can be used to verify past state changes and to query account balances as well as transfers.
...
The L2 node is the interface through which external clients (@cli) can submit deposits, transfers, or withdrawals of funds. Moreover, it is connected to a data store (@data-store) to persist the account balances, whose state representation #footnote[The state representation is the Merkle root, see @glossary.] is maintained on-chain. State transitions of the account balances are verified and executed on-chain, requiring the node to create respective transactions on L1 using the L1's software development kit (SDK). These transactions, in turn, call the respective endpoints of smart contracts described in @contracts to do so. To execute batch transfers the node utilizes a proving system provided by the Prover service (@prover). For deposits and withdrawals, the node creates according Merkle tree updates of the account balances @merkle-tree-updates.
...
The data store is a persistent storage that stores the performed transfers and the account balances whose state representation is stored on the blockchain. To achieve more failsafe and reliable availability of the data, it is replicated sufficiently often by so-called standby's (replicas). In the case of failure, standbys can be used as new primary stores (@data-redundancy).

=== L1 State/ Verifier Contract <contracts>

The L1 State and Verifier Contracts are responsible for verifying and performing updates of the Merkle root of account balances. They can be two separate contracts or a single contract with several endpoints. The important thing is that the state update only happens if the updated state was verified successfully beforehand. The contracts are called by the L2 node by creating according transactions and submitting them to an L1 node.
...
#table(
  ...
  [get_balance],[accountId: AccountID],[balance: UnsignedINT],[Returns a user's L2 account balance. The `UnsignedINT` type should be a type that allows for safe money computations (Read @safe-money) ],
  ...
  [verify],[proof: Proof, public_nputs: PublicInputs],[result: VerifyResult],[Returns whether a proof is legitimate or not]
)
...
#table(
  columns: (auto, auto, auto, auto),
  [Name],[Arguments],[Return Value],[Description],
  [verify_deposit],[depositor: AccountID, amount: UnsignedINT, token_id: TokenID, updated_account_balances: MerkleRootHash, update_metadata: MerkleRootUpdateMetadata, signature: Signature],[],
  [- Verifies that the transaction cointains the same amount of tokens used to update the account balance \
  - Moves the amount from the depositor's L1 account to the account owned by the system \
  ...
  [verify_withdraw],[withdrawer: AccountID, amount: UnsignedINT, tokenId: TokenID, updated_account_balances: MerkleRootHash, update_metadata: MerkleRootUpdateMetadata, signature: Signature],[],
  [- Verifies that the transaction moves the same amount of tokens used to update the account balance \
  - Moves the amount from the account owned by the system to the withdrawer's L1 account \
  - Verifies the withdrawer's signature \
  - Verifies the new Merkle root given the public inputs and metadata \
  - Updates the system's on-chain state on successfull verification
  ],
  [verify_transfers],[proof: Proof, publicInputs: PublicInputs],[],
  [- Verifies that the proof computed from performing batch transfers on the L2 is valid \
  - Updates the system's on-chain state on successfull verification
  ]
)
...
In the first phase (@deposit-client-submit) users submit their deposit requests through the client CLI to the L2 node. The L2 node creates an L1 transaction (_Deploy_) containing the according Merkle root update and the update metadata, which can be used to verify that the update was done correctly. More precisely, the _Deploy_ contains a _Session_ that executes the validation of the Merkle tree update, performs the state transition and transfer the funds from the users L1 account (purse) to the systems L1 account. This _Deploy_ needs to be signed by the user before submitting, this is achieved by making use of the L1's SDK on the client-side.

After submitting, the L1 smart contracts take care of validating the new Merkle root, updating the system's state, and transferring the funds (@deposit-deploy-execution).

Lastly (@deposit-notify), the L2 node gets notified after the _Deploy_ was processed successfully. The node then commits the updated state to the data store. After sufficient time has passed, the user can query its account balance using the client CLI.
...
In the first phase (@transfer-submit) users submit their transfer requests through the client CLI to the L2 node. The L2 node accumulates the transfer requests and checks for independence. In addition, the L2 node will check that the batch proof which is going to be computed next, a valid nonce.

After `t` seconds or `n` transactions (@transfer-prove), the L2 node batches the transfers, creates a proof of computation, and the according _Deploy_ which will execute the validation and the state transition on-chain.

After submitting, the L1 smart contracts take care of first validating the proof and updating the state (@transfer-execute).

Lastly (@transfer-notify), the L2 node gets notified when the _Deploy_ was executed successfully. The node then commits the updated state to the data store. After sufficient time has passed, the users can query their account balances using the client CLI
...
- Nonce/ Kairos counter: A mechanism that prevents the usage of L2 transactions more than once without the user's permission. It is added to each L2 transaction, which is verified by the batch proof and L1 smart contract. For an in-depth explanation, see @uniqueness.
